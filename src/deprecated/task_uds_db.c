...
static struct uds_session_t session[MAX_CLIENT_DB];
...
static int handle_client(struct uds_session_t* uds_session) 
{
	...
	rc = db->uds_server_recv(socket_fd, req, &uds_session->cli_addr, &uds_session->cli_addr_len);
	...
	log->debug(0x01, "db req- fn: %x, cmd: %x, ds: %ld\n", req->fn, req->cmd, req->data_size);

	queue_msg.index = uds_session->index;
	queue_msg.magic = uds_session->magic;
	strcpy(queue_msg.src, QUEUE_NAME_UDS_DB);
	strcpy(queue_msg.dst, QUEUE_NAME_MAIN_DB);

	mq->set_to(queue_msg.dst, &queue_msg);
	return 0;
}

void* task_uds_db_recv(void* ptr)
{
	uint32_t i = 0;
	uint32_t magic = 0;
	...
	socket_fd = db->uds_server_create();
	if(socket_fd < 0) {
	...
	db->uds_close(socket_fd);

	return NULL;
}

void* task_uds_db_send(void* ptr)
{
    struct queue_msg_t queue_msg;
